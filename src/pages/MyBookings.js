...
export default function MyBookings() {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      const res = await axios.get(`https://movierulz1-mjpd.onrender.com/api/tickets/my/${userId}`);
  ...
  const cancelTicket = async (id) => {
    ...
    try {
      await axios.put(`https://movierulz1-mjpd.onrender.com/api/tickets/cancel/${id}`);
    ...
    <div className="container">
      ...
      {tickets.map(ticket => (
        <div key={ticket._id} className="booking-card">
          <img src={`https://movierulz1-mjpd.onrender.com${ticket.poster}`} alt={ticket.movieTitle} />
