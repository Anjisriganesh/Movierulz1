...
export default function MyBookings() {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      const res = await axios.get(`http://localhost:8600/api/tickets/my/${userId}`);
  ...
  const cancelTicket = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:8600/api/tickets/cancel/${id}`);
    ...
    <div className="container">
      ...
      {tickets.map(ticket => (
        <div key={ticket._id} className="booking-card">
          <img src={`http://localhost:8600${ticket.poster}`} alt={ticket.movieTitle} />
